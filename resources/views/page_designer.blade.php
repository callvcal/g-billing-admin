...
@section('main')
    ...
    <div id="page" style="width:100%;height:{{ $doc->settings->ratio }}vw;">
        ...
        @foreach ($doc->items as $item)
            <div class="item item_{{ $item->id }} {{ $item->type }}"
                style="width:{{ $item->pwidth }}vw;height:{{ $item->pheight }}vw;left:{{ $item->px }}vw;top:{{ $item->py }}vw;">
                <div class="content">
                    ...
                    @if ($data)
                        @if ($item->type == 'images' || $item->type == 'inline_gallery')
                            @if (count($data['images']) > 1)
                                <div class="splide splide_{{ $item->type }}" data-width="{{ $item->pwidth }}"
                                    data-height="{{ $item->pheight }}"
                                    data-mobile_height="{{ round(($item->pheight / $item->pwidth) * 100) }}vw">
                                        ...
                                        <ul class="splide__list">
                                            @foreach ($data['images'] as $img)
                                                <li class="splide__slide">
                                                    @if ($item->type == 'images')
                                                        <a class="glightbox"
                                                            href="/storage/{{ str_ireplace('.jpg', '-large.jpg', $img) }}">
                                                    @endif
                                                    <img data-src="/storage/{{ str_ireplace('.jpg', '-medium.jpg', $img) }}"
                                                        class="lazy @if ($loop->first) first @endif">
                                                    @if ($item->type == 'images')
                                                        </a>
                                                    @endif
                                                </li>
                            ...
                            @else
                                @if ($item->type == 'images')
                                    <a class="glightbox"
                                        href="/storage/{{ str_ireplace('.jpg', '-large.jpg', $data['images'][0]) }}">
                                @endif
                                <img data-src="/storage/{{ str_ireplace('.jpg', '-medium.jpg', $data['images'][0]) }}"
                                    @if ($item->type == 'images') data-fslightbox="lightbox" @endif class="lazy first">
                                @if ($item->type == 'images')
                                    </a>
                                @endif
                            @endif
                        ...
                        @if ($item->type == 'video')
                            <a class="glightbox" href="storage/{{ $data['video'] }}">
                                @if (!empty($data['thumb_video']))
                                    <video class="lazy" width="100%" height="100%" autoplay muted loop
                                        id="video_{{ $item->id }}" data-src="storage/{{ $data['thumb'] }}"
                                        style="background:#000">
                                        <source data-src="storage/{{ $data['thumb_video'] }}" type="video/mp4">
                                    </video>
                                @else
                                    <img class="lazy"
                                        data-src="/storage/{{ str_ireplace('.jpg', '-medium.jpg', $data['thumb']) }}">
                                @endif
