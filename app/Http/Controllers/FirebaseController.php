...
class FirebaseController extends Controller
{
    ...
    function sendFcmMessages($fcms, $data, $notification)
    {
        try {
            ...
            Log::channel('callvcal')->error("Error while sendFcmMessages: " . $e->getMessage());
            ...
            // Optionally, log the entire exception object as JSON
            Log::channel('callvcal')->error("Exception details: " . json_encode($e));
    ...
    function sendFcmMessage($fcm_token, $data, $notification)
    {
        ...
        try {
            ...
            Log::channel('callvcal')->info("sendFcmMessage sent" . " " . json_encode($sent));

            return $sent;
        } catch (Exception $e) {
            Log::channel('callvcal')->error("Error while sendFcmMessage: " . $e->getMessage());
            Log::channel('callvcal')->error("Error code: " . $e->getCode());
            Log::channel('callvcal')->error("File: " . $e->getFile());
            Log::channel('callvcal')->error("Line: " . $e->getLine());
            Log::channel('callvcal')->error("Stack trace: " . $e->getTraceAsString());
    ...
    function sendFcmToTopic($topic, $data, $notification)
    {
        ...
        try {
            ...
            Log::channel('callvcal')->info("sendFcmToTopic sent" . " : input:" .json_encode($notification).', res: '. json_encode($sent));
    ...
    {
        Log::channel('callvcal')->info("validateTokenId start :" . date("Y/m/d H:i:s"). " data: ".(file_get_contents(storage_path("app/fcm.json"))));
